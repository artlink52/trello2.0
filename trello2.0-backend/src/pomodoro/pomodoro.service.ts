...
export class PomodoroService {
	...
	async create(userId: string) {
		...
		// нормализуем nullable -> number
		const intervalCount = user.intervalCount ?? 0 // или поставьте ваш дефолт, например 4
		// (по желанию) валидация
		if (intervalCount < 0)
			throw new BadRequestException('intervalCount must be >= 0')
